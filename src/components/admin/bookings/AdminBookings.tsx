...
const AdminBookings = () => {
	...
	return (
		<li className="grid grid-cols-1 gap-4 lg:grid-cols-4 md:grid-cols-2 md:gap-8 my-20">
			{bookingsData && bookingsData?.length > 0 ? bookingsData?.sort((a, b) => new Date(a.checkInDate).getTime() - new Date(b.checkInDate).getTime()).map((booking) => {
				return (
					<>
						{bookingsData && bookingsData?.map((booking, idx) => (
							<a href={`/manage-booking/${booking.id}`} key={booking?.id} className="flex max-w-xs flex-col gap-4 rounded-xl bg-white/10 p-2 text-white hover:bg-white/20">
